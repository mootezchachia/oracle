...
const APP_BASE_URL = "https://oracle-psi-orpin.vercel.app";

async function fetchAppJSON(path) {
  try {
    const r = await fetch(`${APP_BASE_URL}${path}`, { signal: AbortSignal.timeout(12000) });
    if (!r.ok) return null;
    return await r.json();
  } catch { return null; }
}
...
async function callClaude(systemPrompt, userPrompt, maxTokens = 3000) {
  if (!ANTHROPIC_API_KEY) return "LLM unavailable — set ANTHROPIC_API_KEY for AI analysis.";
...
async function aiAnalyzeMarket(market, newsItems, redditPosts, signals) {
  const sys = "You are a superforecaster. Be precise, calibrated, concise. Output ONLY valid JSON.";
  const relevantNews = (newsItems || [])
    .filter(n => market.question?.toLowerCase().split(" ").some(w => w.length > 4 && (n.title || "").toLowerCase().includes(w)))
    .slice(0, 5).map(n => `- ${n.title}`).join("\n") || "No directly relevant news.";

  const relevantReddit = (redditPosts || [])
    .filter(p => market.question?.toLowerCase().split(" ").some(w => w.length > 4 && (p.title || "").toLowerCase().includes(w)))
    .slice(0, 5).map(p => `- [${p.score || 0}pts] ${p.title}`).join("\n") || "No relevant posts.";

  // Run all 4 agents in one batched prompt for speed
  const prompt = `Analyze this prediction market with 4 independent forecasting perspectives.

MARKET: ${market.question}
YES price: ${market.yes_price} (${Math.round(market.yes_price * 100)}% implied probability)
...
{
  "base_rate": {"probability": 0.XX, "confidence": "low|medium|high", "reasoning": "1 sentence on historical base rates"},
  "causal": {"probability": 0.XX, "confidence": "low|medium|high", "reasoning": "1 sentence on causal drivers from news"},
  "adversarial": {"probability": 0.XX, "confidence": "low|medium|high", "reasoning": "1 sentence challenging the consensus"},
  "crowd": {"probability": 0.XX, "confidence": "low|medium|high", "reasoning": "1 sentence on crowd sentiment"},
  "final_probability": 0.XX,
  "signal": "BUY YES|BUY NO|HOLD",
  "thesis": "2 sentence investment thesis"
}`;

  const raw = await callClaude(sys, prompt, 1500);
  return parseJSON(raw);
}

async function handleAI(req, res) {
  // Gather intelligence from our own endpoints
  const [news, reddit, signals, forecasts, strategy, portfolio] = await Promise.all([
    fetchAppJSON("/api/news"),
    fetchAppJSON("/api/reddit"),
    fetchAppJSON("/api/signals"),
    redisGet(FORECAST_KEY),
    fetchAppJSON("/api/strategy100"),
    fetchAppJSON("/api/portfolio"),
  ]);

  // Select top markets to analyze (from algo forecasts)
  const marketsToAnalyze = (forecasts?.top_forecasts || [])
    .slice(0, 10)
    .map(f => f.market)
    .filter(Boolean);

  // Run AI analysis (sequential to respect rate limits)
  const analyses = [];
  for (const market of marketsToAnalyze) {
    try {
      const result = await aiAnalyzeMarket(market, news, reddit, signals);
      const edge = (result.final_probability || 0.5) - market.yes_price;
      analyses.push({
        slug: market.slug,
        question: market.question,
        market_price: market.yes_price,
        ai_probability: result.final_probability,
        edge_pct: Math.round(edge * 10000) / 100,
        signal: result.signal || "HOLD",
        thesis: result.thesis,
        agents: [
          { name: "base_rate", ...result.base_rate },
          { name: "causal", ...result.causal },
          { name: "adversarial", ...result.adversarial },
          { name: "crowd", ...result.crowd },
        ],
      ...
      analyses.push({ slug: market.slug, question: market.question, error: err.message, signal: "ERROR" });
    }
  }

  // Position health check (batch all positions in one call)
  let healthChecks = [];
  const allPositions = [
    ...(strategy?.positions || []),
    ...(portfolio?.positions || []),
  ].filter(p => p.status !== "closed").slice(0, 15);

  if (ANTHROPIC_API_KEY && allPositions.length > 0) {
    const posList = allPositions.map(p =>
      `- "${p.question}" | ${p.side} @ ${p.entry_price} | PnL: ${p.pnl != null ? `$${p.pnl}` : "?"}`
    ).join("\n");
    const newsText = (news || []).slice(0, 15).map(n => `- ${n.title}`).join("\n");
    try {
      const raw = await callClaude(
        "You are a risk manager. Output ONLY a JSON array.",
        `Check positions against news. Most should be "hold". Only flag "alert"/"exit" if news DIRECTLY contradicts.\n\nPOSITIONS:\n${posList}\n\nNEWS:\n${newsText}\n\nRespond: [{"question":"...","status":"hold|alert|exit","reason":"brief"}]`,
        2000
      );
      try { healthChecks = JSON.parse(raw.match(/\[[\s\S]*\]/)?.[0] || "[]"); } catch {}
    } catch {}
  }

  // Generate brief
  const strat = strategy?.account || {};
  const main = portfolio?.account || {};
  const opps = analyses.filter(a => a.signal && a.signal !== "HOLD" && a.signal !== "ERROR");
  const alerts = healthChecks.filter(h => h.status === "alert" || h.status === "exit");

  let brief = `=== ORACLE AI BRIEF ===\n`;
  brief += `${new Date().toISOString()}\n`;
  brief += `$10K: $${main.total_value?.toFixed(0) || "?"} (${main.pnl >= 0 ? "+" : ""}$${main.pnl?.toFixed(0) || "?"}) | `;
  brief += `$1K: $${strat.total_value?.toFixed(0) || "?"} (${strat.total_return >= 0 ? "+" : ""}$${strat.total_return?.toFixed(0) || "?"})\n`;
  brief += `Analyzed: ${analyses.length} markets | Signals: ${opps.length} | Alerts: ${alerts.length}\n\n`;

  for (const o of opps.slice(0, 5)) {
    brief += `[${o.signal}] ${o.question}\n`;
    brief += `  Market: ${Math.round(o.market_price * 100)}c → AI: ${Math.round((o.ai_probability || 0) * 100)}c (edge ${o.edge_pct > 0 ? "+" : ""}${o.edge_pct}%)\n`;
    brief += `  ${o.thesis || ""}\n\n`;
  }
  for (const a of alerts) {
    brief += `!! ${a.status.toUpperCase()}: ${a.question} — ${a.reason}\n`;
  }
